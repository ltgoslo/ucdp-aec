...
class ListPredictParser(TreePredictParser):
    def decode(self, gold_list, pred_list, text_list=None, raw_list=None) -> Tuple[List[Dict], Counter]:
        counter = Counter()
        well_formed_list = []

        for gold, pred in zip(gold_list, pred_list):
            gold_tree, gold_roles = self.tree_of_text(gold)
            pred_tree, pred_roles = self.tree_of_text(pred)

            counter["gold_tree"] += 1
            counter["pred_tree"] += 1
            if len(pred_roles) == len(gold_roles) == len(UCDP_FIELDS):
                counter["well-formed"] += 1
            else:
                counter["ill-formed"] += 1

            well_formed_list.append({
                "text": None,
                "raw_data": None,
                "gold": gold,
                "pred": pred,
                ...
                "gold_event": [("UCDP", "")],
                "pred_event": [("UCDP", "")],
                "gold_role": gold_roles,
                "pred_role": pred_roles,
                "gold_regord": [{"roles": gold_roles.copy(), "type": "UCDP", "trigger": ""}],
                "pred_regord": [{"roles": pred_roles.copy(), "type": "UCDP", "trigger": ""}],
            })

        return well_formed_list, counter

    def tree_of_text(self, text):
        tree = ParentedTree("UCDP", [])
        roles = []
        for pair in text.split("<extra_id_1> <extra_id_0>"):
            pair = pair.replace("<extra_id_0>", "").replace("<extra_id_1>", "").strip()
            field, role = None, None
            for crole, cfield, _ in sorted(UCDP_FIELDS, key=lambda field: len(field[1]), reverse=True):
                if pair.startswith(cfield):
                    field, role = cfield, crole
                    break
            if field is not None:
                argument = pair[len(field):].strip()
                roles.append(("UCDP", role, argument))
                tree.append(ParentedTree(role, [argument]))
        return tree, roles
