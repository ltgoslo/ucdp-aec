...
def get_constraint_decoder(tokenizer, type_schema, decoding_schema, source_prefix=None):
    ...
    elif decoding_schema == 'list':
        return ListConstraintDecoder(tokenizer=tokenizer, type_schema=type_schema, source_prefix=source_prefix)
...
class ListConstraintDecoder(TreeConstraintDecoder):
    def check_state(self, tgt_generated):
        if tgt_generated[-1] == self.tokenizer.pad_token_id:
            return 'start', -1
