...
class GenerativeModel(nn.Module):
    ...
    def forward(self, batch):
        outputs = self.model(input_ids=batch.enc_idxs[:, :self.max_length],
                             attention_mask=batch.enc_attn[:, :self.max_length],
    ...
    def predict(self, batch, num_beams=4, max_length=50):
        self.eval()
        with torch.no_grad():
            outputs = self.model.generate(input_ids=batch.enc_idxs[:, :self.max_length],
                                          attention_mask=batch.enc_attn[:, :self.max_length],
                                          num_beams=num_beams, 
