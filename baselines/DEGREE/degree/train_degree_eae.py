...
elif config.dataset == "ucdp":
    from template_generate_ucdp import eve_template_generator
    template_file = "template_generate_ucdp"
...
for epoch in range(1, config.max_epoch+1):
    ...
    dev_scores = {
        ...
        'aec_agg': aec_agg / aec_cnt
    }

    # print scores
    print("---------------------------------------------------------------------")
    print('AEC Aggregate: {:5.2f}'.format(dev_scores['aec_agg']))
    ...
    if dev_scores['aec_agg'] > best_dev_scores['aec_agg']:
        best_dev_flag = True
